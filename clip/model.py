...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, acquirer=False, lang=None):
        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        if acquirer:
            if self.m_acquirer:
                x = self.acquirer[lang](x)
            else:
                x = self.acquirer(x)
        return x
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_all_layers=False, acquirer=False, lang=None):
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if self.acquirer:
            ...
            # self.multilingual_embedding = nn.Embedding(30514, 512)
    ...
    def encode_text(self, text, layers=None, acquirer=False, tokenize=False, mul_toker=None, lang=None, device='cuda'):
    # def encode_text(self, text, layers=None):
        # if lang == 'en':
        #     x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        # else:
        #     x = self.multilingual_embedding(text).type(self.dtype)
        if tokenize:
            text = list(text)
            if acquirer:
                # import pdb;pdb.set_trace()
                if mul_toker:
                    text = mul_toker(text, truncate=True).to(device)
                else:
                    text = m_toker(text, padding='max_length', max_length=77, truncation=True, return_tensors='pt')['input_ids'].to(device)
                x = self.multilingual_embedding(text).type(self.dtype)
                x = self.multilingual_embedding_linear(x)
                # x = self.attn_embed(x)
            else:
        ...
        if layers == None:
            x = x.permute(1, 0, 2)  # NLD -> LND
            x = self.transformer(x, acquirer=acquirer, lang=lang)
            # x = self.transformer(x)
            x = x.permute(1, 0, 2)  # LND -> NLD
...
def build_model(state_dict: dict, acquirer=False, d_acquirer_hidden=256, m_acquirer=False, langs=None, skip=False, init_mbert_embedding=True):
    ...
    model = CLIP(
        ...
        m_acquirer=m_acquirer, langs=langs, skip=skip, init_mbert_embedding=init_mbert_embedding
    )
